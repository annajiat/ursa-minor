...
class Profile extends Component {
  ...
  setUserData = props => {
    ...
    if (props.user && props.user.uid === requestedUid) {
      ...
      props.actions.loadSelectedUser(requestedUid, user => this.setState(user))
    }
  }
  ...
  render() {
    const { email, name, description, website, photo, recentList, contributorType, otherContributor } = this.state
    const isUser = this.props.user && this.props.user.uid === this.props.match.params.id

    return (
      <AppGrid title={ isUser ? 'My Profile' : 'Profile' } style={{ justifyContent: 'space-between', marginBottom: '100px' }}>
          ...
          <div className="control-panel__group">
            <div className="form__field">
              ...
              <ShowOnly if={ contributorType === 'Other' }>
                ...
                <TextInput value={ otherContributor } disabled={ !isUser } placeholder="Please specify" onChange={ this.updateInputField('otherContributor') } />
              </ShowOnly>
            </div>
          </div>
          <div className="control-panel__group">
          ...
          { isUser
            ? (
              <React.Fragment>
                <div className="control-panel__group">
                  <div className="form__field">
                    <p className="form__label">Confirm current password to save changes</p>
                    <TextInput
                      type="password"
            ...
            : <Link to="/" className="outlined-button outlined-button--link">BACK TO MAP</Link>
          }
        </Grid>
        <Grid item xs={ 12 } sm={ 3 } style={{ textAlign: 'center' }}>
          <div style={ styles.logoContainer }>
            <div style={ styles.logoSpacer }>
              <img src={ isUser ? this.props.user.photo : photo } style={ styles.image } alt="" />
            </div>
          </div>
          <ShowOnly if={ isUser }>
            <ReactFilestack
